...
inline int readIndexInput(const std::string& prompt, int minInterval, int maxInterval) {
    int value;
    std::cout << prompt;
    if (!(std::cin >> value)) {
        std::cin.clear();
...
inline std::string readStringInput(const std::string& prompt) {
    std::string value;
    ...
    while (!valueIsValid) {
        ...
        if (std::cin.fail()) {
            ...
            throw std::invalid_argument("TILTOTT KARAKTERT HASZNALTAL: | vagy $");
        } else {
            valueIsValid = true;
        }
    }
